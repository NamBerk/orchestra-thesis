...
void broadcast_recv(struct broadcast_conn *c, const linkaddr_t *from) {
  ...
  if (mode != MODE_SOURCE && rank_coeff_matrix != FULL_RANK) {
    nack_multiplier = 1;
...
void init_rnc(void) {
  ...
  uint8_t id_sinks[] = ID_SINKS;
  uint8_t id_relay_sinks[] = ID_RELAY_SINKS;
  if (node_id == ID_SOURCE) {
    mode = MODE_SOURCE;
    local_batch_id = 0xff;
  } else if (array_contains(node_id, id_sinks, sizeof(id_sinks))) {
    mode = MODE_SINK;
...
void send_new_packet(void *bid) {
  ...
  print_rnc_packet("SOURCE INITIAL BROADCAST", &p);
...
void process_data(struct rnc_pkt *p_recv) {
  ...
  if (local_batch_id < p_recv->batch_id) {
    if (rank_coeff_matrix != FULL_RANK &&
        (mode == MODE_SINK || mode == MODE_RELAY_PLUS_SINK)) {
  ...
  if (rank_coeff_matrix != FULL_RANK) {
    ...
    */
    ...
    if (rank_coeff_matrix == FULL_RANK) {
      if (mode == MODE_SINK || mode == MODE_RELAY_PLUS_SINK) {
#if GF != 2
